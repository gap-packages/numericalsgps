...
##
## Returns the least numerical semigroup containing the list elts of elements and having and with the largest possible Frobenius number not greater than frob.
...
##
InstallGlobalFunction(ObviouslyForcedGaps, function(PF)  
  ...
  Info(InfoTipo,2,"\"obviously\" forced gaps:\n",forced_gaps,"\n");
  return forced_gaps;
end);
...
######################################
##
#F SimpleForcedIntegers (f_gaps,f_elts,PF)
##
...
InstallGlobalFunction(SimpleForcedIntegers, function(f_gaps,f_elts,PF)
...
#############################################
InstallGlobalFunction(ForcedGapsAndElementsForPseudoFrobenius, function(arg)
  ...
  ##################
  forced_gaps := ObviouslyForcedGaps(PF);
  ##################
  # elements forced by:
  ...
  extra_forced_gaps := function(f_ints)
    ...
    for v in free do
      if v in sfree then
        left := SimpleForcedIntegers(f_g,Union(f_e,[v]),PF);
        if left = fail then
  ...
  repeat
    ...
    forced_integers := SimpleForcedIntegers(f_gaps,f_elts,PF);
...
InstallGlobalFunction(NumericalSemigroupsWithPseudoFrobeniusNumbers, function(arg)
  local  tree, path, nodes_visited, dead_nodes, opt_draw_tree, PF, type, 
         frob, initially_forced_integers, freeElementsTree_recursive, 
  ...
  # forced integers
  initially_forced_integers := ForcedGapsAndElementsForPseudoFrobenius(PF);
  ...
  freeElementsTree_recursive :=  function(fg,fe)
    ...
    while Length(current_free) > 1 do
      nodes_visited := nodes_visited + 1;
      v := current_free[1];
      left := SimpleForcedIntegers(nfg,Union(fe,[v]),PF);
      if left = fail then
        right := SimpleForcedIntegers(Union(nfg,[v]),fe,PF);
...
InstallGlobalFunction(RandomNumericalSemigroupWithPseudoFrobeniusNumbers, function(arg)
  ...
  # forced integers
  f_ints := ForcedGapsAndElementsForPseudoFrobenius(PF);
  ...
  for i in [1..m_att] do
    while free <> [] do
      v := RandomList(free);
      nfig := SimpleForcedIntegers(Union(f_ints[1],[v]),f_ints[2],PF);
      if nfig <> fail then
        if IsRange(Union(nfig)) then
          return NumericalSemigroupByGaps(nfig[1]);
        fi;
      fi;
      nfie := SimpleForcedIntegers(f_ints[1],Union(f_ints[2],[v]),PF);
      if nfie <> fail then
